...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUsers = useCallback(async (params: { page?: number; search?: string; append?: boolean; pageSize?: number } | number = {}) => {
    const actualParams = typeof params === 'number' ? { page: params } : params;
    const { page = 1, search = '', append = false, pageSize = 10 } = actualParams;
  ...
  return (
    <UserContext.Provider value={{ users, total, loading, fetchUsers, setUsers }}>
      {children}
